    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-3 lg:h-[calc(100vh-135px)] min-h-0">
        ...
        <div class="xl:col-span-7 bg-white dark:bg-ink-900 rounded-xl border border-gray-200 dark:border-ink-700 shadow-sm overflow-hidden flex flex-col min-h-0">
            ...
            <div class="p-3 space-y-2 shrink-0">
                ...
                {{-- Campos dinâmicos baseados no tipo de negócio --}}
@if($pdvConfig['tipo_venda_padrao'] == 'peso')
    {{-- Campo de peso para sorveteria --}}
    <div class="col-span-12 mt-2">
        <div class="relative">
            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-ink-400 text-xs">⚖️</span>
            <input type="text" 
                wire:model="peso" 
                placeholder="Peso em kg (ex: 0.500)"
                class="w-full pl-8 pr-3 py-2 border border-gray-200 dark:border-ink-700 rounded-lg text-sm">
        </div>
        <p class="text-xs text-ink-500 mt-1">Informe o peso do produto (ex: 0.500 = 500g)</p>
    </div>
@endif

@if($pdvConfig['permite_meio'])
    {{-- Botão de meia porção para pizzaria/açaí --}}
    <div class="col-span-12 mt-2">
        <div class="flex items-center justify-between p-2 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
            <span class="text-sm font-medium">Meia porção?</span>
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" wire:model="meiaPorcao" class="sr-only peer">
...
@if($pdvConfig['exibir_tamanhos'])
    {{-- Seleção de tamanhos para pizzaria/sorveteria --}}
    <div class="col-span-12 mt-2">
        <label class="block text-xs font-semibold text-ink-500 mb-1">Tamanho</label>
        <div class="grid grid-cols-3 gap-2">
            @foreach($tamanhosDisponiveis as $tamanho)
                <button wire:click="$set('tamanhoSelecionado', '{{ $tamanho }}')"
                    class="px-3 py-2 rounded-lg border-2 text-sm transition-all
                        {{ $tamanhoSelecionado == $tamanho ? 'border-primary-600 bg-primary-50 text-primary-600' : 'border-gray-200 hover:border-primary-200' }}">
                    {{ $tamanho }}
                </button>
            @endforeach
        </div>
...
@if($pdvConfig['exibir_adicionais'])
    {{-- Adicionais para lanchonete --}}
    <div class="col-span-12 mt-2">
        <label class="block text-xs font-semibold text-ink-500 mb-1">Adicionais</label>
        <div class="space-y-1">
            @foreach($adicionaisDisponiveis as $adicional)
                <label class="flex items-center gap-2 p-2 border rounded-lg cursor-pointer hover:bg-gray-50">
                    <input type="checkbox" wire:model="adicionaisSelecionados" value="{{ $adicional['nome'] }}">
                    <span class="text-sm">{{ $adicional['nome'] }}</span>
                    <span class="text-xs text-primary-600 ml-auto">+ R$ {{ number_format($adicional['preco'], 2, ',', '.') }}</span>
                </label>
            @endforeach
        </div>
    </div>
